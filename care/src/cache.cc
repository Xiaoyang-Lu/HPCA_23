...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
           ...
           if (cache_type == IS_LLC)
 	{
		    ...
		    if (active_cycles_per_core[fill_cpu].find(now) != active_cycles_per_core[fill_cpu].end())
		    {
			total_overlapped_cycles_per_core[fill_cpu]++;	
		    }
...
void CACHE::handle_writeback()
{
            ...
            if (cache_type == IS_LLC)
	{
		...
		if (active_cycles_per_core[writeback_cpu].find(now) != active_cycles_per_core[writeback_cpu].end())
                {
                        total_overlapped_cycles_per_core[writeback_cpu]++;
                }
...
void CACHE::handle_read()
{
	...
	{   
                while (now <= current_core_cycle[read_cpu] + LATENCY)
                {
               	...
               	if (active_cycles_per_core[read_cpu].find(now) != active_cycles_per_core[read_cpu].end())
                {
                        total_overlapped_cycles_per_core[read_cpu]++;
                }
...
void CACHE::handle_prefetch()
{
	...
	{ 
	       while (now <= current_core_cycle[prefetch_cpu] + LATENCY)
                ...
                if (active_cycles_per_core[prefetch_cpu].find(now) != active_cycles_per_core[prefetch_cpu].end())
                {
                        total_overlapped_cycles_per_core[prefetch_cpu]++;
                }
